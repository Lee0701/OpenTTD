	...
	for (; v != NULL; v = v->Next()) {
		...
		if (HasBit(v->vehicle_flags, VF_CARGO_UNLOADING) && (u->current_order.GetUnloadType() & OUFB_NO_UNLOAD) == 0) {
			...
			if (CargoHasDestinations(v->cargo_type)) {
				 ...
				 * accepted, otherwise transfer. If the current order is forced unload,
				 * always unload all cargo. */
				VehicleCargoList::MoveToAction mta = HasBit(ge->acceptance_pickup, GoodsEntry::ACCEPTANCE) ? VehicleCargoList::MTA_FINAL_DELIVERY : VehicleCargoList::MTA_TRANSFER;
