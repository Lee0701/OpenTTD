...
struct GraphLegendWindow : Window {
	GraphLegendWindow(WindowDesc *desc, WindowNumber window_number) : Window(desc)
	{
		...
		for (CompanyID c = COMPANY_FIRST; c < MAX_COMPANIES; c++) {
			if (!_legend_excluded_companies.at(c)) this->LowerWidget(WID_GL_FIRST_COMPANY + c);
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		...
		SetDParam(0, cid);
		SetDParam(1, cid);
		DrawString(tr.left, tr.right, CenterBounds(tr.top, tr.bottom, GetCharacterHeight(FS_NORMAL)), STR_COMPANY_NAME_COMPANY_NUM, _legend_excluded_companies.at(cid) ? TC_BLACK : TC_WHITE);
	}
