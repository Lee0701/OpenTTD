	...
	for (auto tile : Map::Iterate()) {
		switch (GetTileType(tile)) {
			...
			case MP_TUNNELBRIDGE:
				if (tile.m1() & 0x80) SetOldTileOwner(tile, OWNER_TOWN);
				break;
	...
	if (IsSavegameVersionBefore(SLV_6, 1)) {
		for (auto t : Map::Iterate()) {
			switch (GetTileType(t)) {
				...
				case MP_ROAD:
					t.m4() |= (t.m2() << 4);
					if ((GB(t.m5(), 4, 2) == ROAD_TILE_CROSSING ? (Owner)t.m3() : GetOldTileOwner(t)) == OLD_OWNER_TOWN) {
						SetTownIndex(t, CalcClosestTownFromTile(t)->index);
					} else {
						SetTownIndex(t, 0);
					}
		...
		for (auto t : Map::Iterate()) {
			switch (GetTileType(t)) {
				...
				case MP_STATION:
					...
					SB(t.m7(), 0, 5, HasBit(t.m6(), 2) ? OWNER_TOWN : GetOldTileOwner(t));
					SB(t.m3(), 4, 4, t.m1());
					t.m4() = 0;
					break;
				...
				case MP_TUNNELBRIDGE:
					...
					if (((old_bridge && IsBridge(t)) ? (TransportType)GB(t.m5(), 1, 2) : GetTunnelBridgeTransportType(t)) == TRANSPORT_ROAD) {
						...
						SB(t.m3(), 4, 4, o == OLD_OWNER_NONE ? OLD_OWNER_TOWN : o); // tram owner
					}
	...
	if (IsSavegameVersionBefore(SLV_83)) {
		for (auto t : Map::Iterate()) {
			if (IsShipDepotTile(t)) {
				t.m4() = (TileHeight(t) == 0) ? OLD_OWNER_WATER : OLD_OWNER_NONE;
			}
		}
	}
