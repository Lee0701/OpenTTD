...
struct YAPFSettings {
	...
	uint16 route_travel_time_factor;         ///< factor * 64 for the link travel time
	uint16 route_station_waiting_factor;     ///< factor * 64 for the waiting cargo at a station
	uint16 route_station_last_veh_factor;    ///< factor * 64 for the time since the last vehicle arrived at a station
...
struct CargodestSettings {
	...
	uint16 weight_scale_ind[2];              ///< weight scale divisor for (1) produced cargo (2) stockpiled cargo
